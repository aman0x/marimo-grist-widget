...
# Create keyward package directory
os.makedirs("/marimo/keyward", exist_ok=True)

# Write keyward __init__.py
with open("/marimo/keyward/__init__.py", "w") as f:
    f.write("""from .api import KeywardApi, api
from .table_operations import (
    get_dataframe, add_records, update_record, delete_records,
    create_table, remove_table, add_column, remove_column,
    get_actions_button, set_table_name, get_table_name,
)
__version__ = "0.1.0"
__all__ = ['KeywardApi', 'api', 'get_dataframe', 'add_records', 'update_record',
    'delete_records', 'create_table', 'remove_table', 'add_column', 'remove_column',
    'get_actions_button', 'set_table_name', 'get_table_name']
""")

# Write keyward table_operations.py
with open("/marimo/keyward/table_operations.py", "w") as f:
    f.write('''import json
import pandas as pd
from typing import Any, Dict, List, Optional, Union
...
def create_table(table_name: str, columns: Dict[str, str]) -> List[List[Any]]:
    ...
    for label, col_type in columns.items():
        field_id = "ID_field" if label.strip().lower() == "id" else label
        actions.append(["AddVisibleColumn", table_name, field_id,
            {"type": col_type, "label": label, "widgetOptions": "", "formula": ""}])
    for action in actions:
        _queue_action(action)
    return actions
...
def add_column(col_id: str, col_type: str = "Text", label: Optional[str] = None, table_name: Optional[str] = None) -> List[Any]:
    table = table_name or _current_table_name
    action = ["AddVisibleColumn", table, col_id,
        {"type": col_type, "label": label or col_id, "widgetOptions": "", "formula": ""}]
    _queue_action(action)
    return action
...
with open("/marimo/keyward/api.py", "w") as f:
    f.write('''import pandas as pd
from typing import Any, Dict, List, Optional
from .table_operations import (
    get_dataframe, add_records as _add_records, update_record as _update_record,
    update_records as _update_records, delete_records as _delete_records,
